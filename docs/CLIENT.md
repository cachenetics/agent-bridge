...
| 200 | `{"ok": true, "relaxed": true, "message_id": "<id>"}` | Posted to a RELAXED (free-chat) channel: relayed as-is, no schema gate. A reply over Discord's 2000-char cap is split into up to `max_chunks` messages (response carries `"chunks": N`, `message_id` is the first); beyond that it rides as a file (`routed_as_attachment`). | Done. |
...
| 422 | `{"ok": false, "reason": "forum requires a tag: ..."}` | A NEW forum post on a require-a-tag forum with no valid tag (the `reason` lists the forum's tag names). The bridge rejects it itself rather than relaying a raw Discord 400. | Add `tags` with at least one of the listed forum tag NAMES and resend. |
